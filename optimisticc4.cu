...
* Jonathan Lehman
* April 18, 2012
* based on each move generation, somewhat naive in that it hopes for the best
*/
...
//cuda functions
__global__ void generateMove(Board *board, int *move, char currentPlayer, int *scoreArray, char *moveArray);
...
void compTurn(Board &board, char currentPlayer){
    ...
    if(!useGPU){
        ...
        do{
            ...
            //TODO: make all these values macros?
            int threadX = tx;
            int threadY = ty;
            ...
            cudaMalloc((void**)&scoreArray, sizeof(int) * gridSize * gridSize);
            cudaMalloc((void**)&moveArray, sizeof(char) * gridSize * gridSize);
            ...
            generateMove<<<grid, block>>>(cudaBoard, cudaMove, currentPlayer, scoreArray, moveArray);
...
__global__
void generateMove(Board *board, int *move, char currentPlayer, int *scoreArray, char *moveArray){
    //shared memory to reduce global mem access, speeds up algorithm
    __shared__ int evalVals[tx][ty];//array to store each threads evaluation
    __shared__ char moves[tx][ty][numMoves];//stores moves generated by block and thread IDs
    __shared__ char bestMove[tx][ty];//stores best move when threads divide work
    __shared__ int bestScore[tx][ty];//stores best scores when threads divide work
    ...
    //bool possible = true;
    evalVals[threadIdx.x][threadIdx.y] = 0;
    ...
    for(int i = 0; i < numMoves; i++){
        ...
        if(devCanMove(newBoard, moves[threadIdx.x][threadIdx.y][i])){
            ...
            if((winner = devCheckWin(newBoard)) == ' '){//no winner
                evalVals[threadIdx.x][threadIdx.y] += devEvaluate(newBoard, maxPlayer);
            }
            else{//winner
                if(winner == maxPlayer){
                    evalVals[threadIdx.x][threadIdx.y] = winScore;//comp wins
                    if(i == 0){
                    ...
                    evalVals[threadIdx.x][threadIdx.y] = -winScore;//opp wins
                    if(i == 1){
            ...
            if(i == 0){
                evalVals[threadIdx.x][threadIdx.y] = -invalidScore;
            }
    ...
    //compare thread scores, to get best score for each block
    //at this point should have correct score in evalVals
    if(threadIdx.x == 0 && threadIdx.y == 0){
        int maxBlockScore = -invalidScore;
        char maxMove;
        
        //iterate through scores per thread, and add to get block total
        for(int i = 0; i < blockDim.x; i++){
            for(int j = 0; j < blockDim.y; j++){
                //set new maxBlockScore if find higher value, and store associated best move
                if(evalVals[i][j] > maxBlockScore){
                    maxBlockScore = evalVals[i][j];
                    maxMove = moves[i][j][0];//first move in sequence of moves (best move to make NEXT)
                }
            }
        }
        //store maxBlockScore (global mem access) and max move
        scoreArray[blockIdx.y * gridDim.x + blockIdx.x] = maxBlockScore;
        moveArray[blockIdx.y * gridDim.x + blockIdx.x] = maxMove;
    }
    
    //synchronize threads to do following calculations and comparisons etc
    __syncthreads();
    
    //split work among all threads in block zero to find max, then find max of these
    if((blockIdx.y * gridDim.x + blockIdx.x) == 0){
        ...
        bestScore[threadIdx.x][threadIdx.y] = -invalidScore;
        
        //spit work among threads adn find sub max's of the global array
        for(int i = start; i < start + work; i++){
            int currScore = scoreArray[i];//so only one global access for this memory instead of 2
            if(currScore > bestScore[threadIdx.x][threadIdx.y]){
                bestScore[threadIdx.x][threadIdx.y] = currScore;
                bestMove[threadIdx.x][threadIdx.y] = moveArray[i];
            }
        }
        ...
        if(threadIdx.x == 0 && threadIdx.y == 0){
            ...
            int bestOverallScore = -invalidScore;
            Board b2;
            transferBoard(&b2, board);
            
            for(int i = 0; i < blockDim.x; i++){
                for(int j = 0; j < blockDim.y; j++){
                    if(bestScore[i][j] > bestOverallScore && devCanMove(b2, i)){
                        bestOverallScore = bestScore[i][j];
                        bestOverallMove = bestMove[i][j];
                    }
                }
            }
            //set final move
